...
- [x] Record YouTube demo video → https://youtu.be/_5ruHnVR9sI
- [x] Create Issue on torvalds/AudioNoise → [#59](https://github.com/torvalds/AudioNoise/issues/59)
- [ ] Effect chaining (multiple effects in series)
- [ ] Real-time MIDI input processing

---

## Bonus: Found a Bug in Linus's Code!

While porting to Wasm, I discovered a **bug in the original phaser code**.

### Symptom
Setting `pot3` to maximum causes audio to go silent (NaN propagation).

### Root Cause
In `phaser.h`, `pot3` is doubled before being cubed:
...
With 4-octave LFO sweep: `2⁴ × 6690 = 107,040Hz` — way above Nyquist (24kHz at 48kHz), causing biquad filter instability.

### Workaround
Clamped `pot3 ≤ 0.5` in the UI. Reported as Issue [#59](https://github.com/torvalds/AudioNoise/issues/59).
