...
Linusの設計では、4つのポットが各エフェクトで異なる役割を持つ（物理的なギターペダルを想定）:

**ギターエフェクト（入力信号を加工）**

| Effect | Pot1 | Pot2 | Pot3 | Pot4 |
|--------|------|------|------|------|
| Phaser | LFO周期 [25-2000ms] | FB [0-75%] | 中心周波数 [50-880Hz] | Q [0.25-2.0] |
| Echo | 遅延 [0-1000ms] | - | LFO深さ [0-4ms] | FB [0-100%] |
| Flanger | LFO速度 [0-10Hz²] | 遅延 [0-4ms] | 深さ [0-100%] | FB [0-100%] |
| Distortion | ドライブ [1-50x] | トーン [1-10kHz] | 出力 [0-100%] | モード |
| Discont | ピッチ比 [0.5-2.0x] | - | - | - |

**シンセエフェクト（入力を無視してテストトーン生成）**

| Effect | Pot1 | Pot2 | Pot3 | Pot4 |
|--------|------|------|------|------|
| FM | 音量 | キャリア [100-8100Hz] | 変調深さ [±1 oct] | 変調周波数 [1-11Hz] |
| AM | 音量 | キャリア [100-8100Hz] | 変調深さ | 変調周波数 [1-11Hz] |

> FM/AMは`convert.c`のテストトーン生成器。入力信号を完全に無視する。
...
```
問題: ScriptProcessorNode はメインスレッドで動く

Main Thread
├── GC (ガベージコレクション)     ← 突然割り込み
├── DOM 描画                      ← 16ms ごと
├── イベント処理                  ← マウス、キーボード
└── ScriptProcessor callback      ← 「バッファ空いた！今すぐ！」
```

データはメモリにあっても、**コールバックが呼ばれるタイミング** が不定。
...
現在の実装 (512サンプル = ~10ms) は **一般リスナーには十分**。

### 正確なスケジューリング

Web Audio API の `setValueAtTime()` を使えば μs 精度で予約可能:
...
ただし Wasm エフェクトと組み合わせるには AudioWorklet が必要。

### Wasm Audio の現状 (2026年)

| 方式 | 精度 | 複雑さ | Wasm連携 |
|------|------|--------|----------|
| ScriptProcessorNode | △ 10-50ms | ◎ 簡単 | ◎ 簡単 |
| AudioWorklet | ◎ <1ms | △ 複雑 | △ 要工夫 |
| AudioWorklet + SharedArrayBuffer | ◎ <1ms | ✗ 難しい | ◎ 高速 |

**結論**: 本格的なDAWレベルの精度には AudioWorklet 移行が必要。
デモ/学習目的なら ScriptProcessorNode で十分。

---

## 次のステップ

- [x] YouTube動画を撮影してREADMEに埋め込み → https://youtu.be/_5ruHnVR9sI
- [x] torvalds/AudioNoise に Issue で報告 → [#59](https://github.com/torvalds/AudioNoise/issues/59)
- [ ] エフェクトチェーン（複数エフェクト直列）
- [ ] MIDI入力のリアルタイム処理
- [ ] AudioWorklet 版の実装（精度向上）

---

## おまけ: Phaserのバグを発見

Wasm移植中に**Linusのコードにバグを発見**してしまった。

### 症状
Phaserで`pot3`を最大にすると音が出なくなる（NaN伝播）。

### 原因
`phaser.h`の`phaser_init()`で`pot3`が2倍されてからcubeされる:

```c
pot3 = 2*pot3;  // pot3=1.0 → 2.0
phaser.center_f = linear(pot3*pot3*pot3, 50, 880);  // 2³=8 → 6690Hz!
```

LFOで4オクターブ上げると `2⁴ × 6690 = 107,040Hz`、Nyquist周波数（24kHz@48kHz）を大幅に超えてbiquadフィルタが発散。

### 対策
UI側で`pot3 ≤ 0.5`に制限して回避。Issue [#59](https://github.com/torvalds/AudioNoise/issues/59) で報告済み。
